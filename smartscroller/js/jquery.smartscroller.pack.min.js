(function($){$.fn.extend({smartscroller:function(options){var defaults={url:"",ajaxOptions:{},viewModel:null,sortControl:null,page:0,itemsPerPage:10,bottomOffset:30,previousLink:null,clientSideSort:null};var opts=defaults;if(options){opts=$.extend(defaults,options)}ko.applyBindings(opts.viewModel);opts.sortControl.change(updateSorting);opts.previousLink.live("click",gotoPrevious);opts.scrollContainer.scroll(scrollCheck);var headPage;var tailPage;this.refresh=function(){opts.viewModel.hasPrevious(false);opts.viewModel.hasNext(false);ajaxRefresh("all",opts.page,function(){headPage=opts.page;tailPage=opts.page})};function scrollCheck(){if(getBottomSpacing()-opts.scrollContainer.scrollTop()<opts.bottomOffset&&opts.viewModel.hasNext()){gotoNext()}}function getBottomSpacing(){return opts.listContainer.height()-opts.scrollContainer.height()}function gotoPrevious(){headPage--;opts.viewModel.hasPrevious(false);var lastListHeight=opts.listContainer.height();ajaxRefresh("previous",headPage,function(){var newScrollPosition=opts.listContainer.height()-lastListHeight;opts.scrollContainer.scrollTop(newScrollPosition)});return false}function gotoNext(){tailPage++;ajaxRefresh("next",tailPage)}function ajaxRefresh(refreshType,page,callback){if(refreshType==="all"||refreshType==="previous"){opts.viewModel.gettingPrevious(true)}else if(refreshType==="next"){opts.viewModel.gettingNext(true)}var data={page:page,itemsPerPage:opts.itemsPerPage,sortOrder:opts.sortControl.find("option:selected").val()};$.extend(data,opts.ajaxOptions);$.ajax({type:"POST",url:opts.url,data:data,success:function(result){if(refreshType==="all"){opts.viewModel.hasPrevious(result.hasPrevious);opts.viewModel.hasNext(result.hasNext);opts.viewModel.gettingPrevious(false);opts.viewModel.data(result.data);callback()}else if(refreshType==="next"){opts.viewModel.hasNext(result.hasNext);opts.viewModel.gettingNext(false);var newData=opts.viewModel.data();for(var index=0;index<result.data.length;index++){newData.push(result.data[index])}opts.viewModel.data(newData)}else if(refreshType==="previous"){opts.viewModel.hasPrevious(result.hasPrevious);opts.viewModel.gettingPrevious(false);var newData=opts.viewModel.data();for(var index=result.data.length-1;index>=0;index--){newData.unshift(result.data[index])}opts.viewModel.data(newData);callback()}}})}function updateSorting(){if(!opts.viewModel.hasPrevious()&&!opts.viewModel.hasNext()){opts.clientSideSort($(this).find("option:selected").val());opts.scrollContainer.scrollTop(0);return}opts.viewModel.hasPrevious(false);opts.viewModel.hasNext(false);ajaxRefresh("all",0,function(){opts.scrollContainer.scrollTop(0);tailPage=0;headPage=0})}return this}})})(jQuery);(function(jQuery,undefined){var oldManip=jQuery.fn.domManip,tmplItmAtt="_tmplitem",htmlExpr=/^[^<]*(<[\w\W]+>)[^>]*$|\{\{\! /,newTmplItems={},wrappedItems={},appendToTmplItems,topTmplItem={key:0,data:{}},itemKey=0,cloneIndex=0,stack=[];function newTmplItem(options,parentItem,fn,data){var newItem={data:data||(data===0||data===false)?data:(parentItem?parentItem.data:{}),_wrap:parentItem?parentItem._wrap:null,tmpl:null,parent:parentItem||null,nodes:[],calls:tiCalls,nest:tiNest,wrap:tiWrap,html:tiHtml,update:tiUpdate};if(options){jQuery.extend(newItem,options,{nodes:[],parent:parentItem})}if(fn){newItem.tmpl=fn;newItem._ctnt=newItem._ctnt||newItem.tmpl(jQuery,newItem);newItem.key=++itemKey;(stack.length?wrappedItems:newTmplItems)[itemKey]=newItem}return newItem}jQuery.each({appendTo:"append",prependTo:"prepend",insertBefore:"before",insertAfter:"after",replaceAll:"replaceWith"},function(name,original){jQuery.fn[name]=function(selector){var ret=[],insert=jQuery(selector),elems,i,l,tmplItems,parent=this.length===1&&this[0].parentNode;appendToTmplItems=newTmplItems||{};if(parent&&parent.nodeType===11&&parent.childNodes.length===1&&insert.length===1){insert[original](this[0]);ret=this}else{for(i=0,l=insert.length;i<l;i++){cloneIndex=i;elems=(i>0?this.clone(true):this).get();jQuery(insert[i])[original](elems);ret=ret.concat(elems)}cloneIndex=0;ret=this.pushStack(ret,name,insert.selector)}tmplItems=appendToTmplItems;appendToTmplItems=null;jQuery.tmpl.complete(tmplItems);return ret}});jQuery.fn.extend({tmpl:function(data,options,parentItem){return jQuery.tmpl(this[0],data,options,parentItem)},tmplItem:function(){return jQuery.tmplItem(this[0])},template:function(name){return jQuery.template(name,this[0])},domManip:function(args,table,callback,options){if(args[0]&&jQuery.isArray(args[0])){var dmArgs=jQuery.makeArray(arguments),elems=args[0],elemsLength=elems.length,i=0,tmplItem;while(i<elemsLength&&!(tmplItem=jQuery.data(elems[i++],"tmplItem"))){}if(tmplItem&&cloneIndex){dmArgs[2]=function(fragClone){jQuery.tmpl.afterManip(this,fragClone,callback)}}oldManip.apply(this,dmArgs)}else{oldManip.apply(this,arguments)}cloneIndex=0;if(!appendToTmplItems){jQuery.tmpl.complete(newTmplItems)}return this}});jQuery.extend({tmpl:function(tmpl,data,options,parentItem){var ret,topLevel=!parentItem;if(topLevel){parentItem=topTmplItem;tmpl=jQuery.template[tmpl]||jQuery.template(null,tmpl);wrappedItems={}}else if(!tmpl){tmpl=parentItem.tmpl;newTmplItems[parentItem.key]=parentItem;parentItem.nodes=[];if(parentItem.wrapped){updateWrapped(parentItem,parentItem.wrapped)}return jQuery(build(parentItem,null,parentItem.tmpl(jQuery,parentItem)))}if(!tmpl){return[]}if(typeof data==="function"){data=data.call(parentItem||{})}if(options&&options.wrapped){updateWrapped(options,options.wrapped)}ret=jQuery.isArray(data)?jQuery.map(data,function(dataItem){return dataItem?newTmplItem(options,parentItem,tmpl,dataItem):null}):[newTmplItem(options,parentItem,tmpl,data)];return topLevel?jQuery(build(parentItem,null,ret)):ret},tmplItem:function(elem){var tmplItem;if(elem instanceof jQuery){elem=elem[0]}while(elem&&elem.nodeType===1&&!(tmplItem=jQuery.data(elem,"tmplItem"))&&(elem=elem.parentNode)){}return tmplItem||topTmplItem},template:function(name,tmpl){if(tmpl){if(typeof tmpl==="string"){tmpl=buildTmplFn(tmpl)}else if(tmpl instanceof jQuery){tmpl=tmpl[0]||{}}if(tmpl.nodeType){tmpl=jQuery.data(tmpl,"tmpl")||jQuery.data(tmpl,"tmpl",buildTmplFn(tmpl.innerHTML))}return typeof name==="string"?(jQuery.template[name]=tmpl):tmpl}return name?(typeof name!=="string"?jQuery.template(null,name):(jQuery.template[name]||jQuery.template(null,htmlExpr.test(name)?name:jQuery(name)))):null},encode:function(text){return(""+text).split("<").join("<").split(">").join(">").split('"').join(""").split("'").join("'");
		}
	});
